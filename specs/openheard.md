...
浏览器侧有四个页面。待确认队列把观测到的对话补成通联。日志列表是正式记录的唯一入口，负责导出 ADIF。快速补录写没有任何观测的通联。公开展示页只读，不暴露写接口，也不显示未确认的内容。
...
- 语音识别跑在守护进程内还是独立服务
- 公开展示页的对外入口。主机在国内而 `bg0cg.ampr.org` 无法备案，所以对外那一面必须落在境外。公开页因此要能脱离管理端单独部署。
